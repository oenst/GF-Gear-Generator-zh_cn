...
class cmdDefPressedEventHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self,args):
        ...
        pitch = inputs.addValueInput('Pitch', '径节 [in]', 'in', adsk.core.ValueInput.createByReal(get(self, 'Pitch', 23.460456)))
...
class cmdDef2PressedEventHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self,args):
        ...
        inputs.addFloatSpinnerCommandInput('PressureAngle', '压力角 [°]', 'deg', 14.5, 30, 0.5, get(self, 'PressureAngle', 14.5))
        inputs.addValueInput('RadialThickness_mm','径向厚度 [mm]','mm', adsk.core.ValueInput.createByReal(get(self, 'RadialThickness_mm', 0.5)))
        inRadialThickness = inputs.addValueInput('RadialThickness_in','径向厚度 [in]','in', adsk.core.ValueInput.createByReal(get(self, 'RadialThickness_in', 0.635)))
...
class cmdDef4PressedEventHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self,args):
        ...
        HelicalSystem.listItems.add('径向\n+保持直齿轮几何/尺寸。\n-需使用专用刀具，每种螺旋角需一种。',True,'Resources/Helical')
        HelicalSystem.listItems.add('法向\n+使用直齿轮刀具。\n-不保持直齿轮几何/尺寸，因此不能直接替换。',False,'Resources/Helical')
...
class cmdDef8PressedEventHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self,args):
        ...
        inputs.addIntegerSpinnerCommandInput('ZWheel', '齿数，大轮 [ ]', 6, 250, 1, get(self, 'ZWheel', 17))
        inputs.addIntegerSpinnerCommandInput('ZPinion', '齿数，小轮 [ ]', 6, 250, 1, get(self, 'ZPinion', 17))
        inputs.addFloatSpinnerCommandInput('PressureAngle', '压力角 [°]', 'deg', 14.5, 30, 0.5, get(self, 'PressureAngle', 14.5))
...
class cmdDef9PressedEventHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self,args):
        ...
        inputs.addBoolValueInput('FastCompute','快速计算', True, '', get(self, 'FastCompute', defaultfc))
        inputs.addFloatSpinnerCommandInput('X','变位系数 "X" [ ]','',-1,1,.01, get(self, 'X', 0))
        inputs.addValueInput('Module', '模数 [mm]', 'mm', adsk.core.ValueInput.createByReal(get(self, 'Module', .03)))
        pitch = inputs.addValueInput('Pitch', '径节 [in]', 'in', adsk.core.ValueInput.createByReal(get(self, 'Pitch', 23.460456)))
        pitch.isVisible = False
...
class cmdDef11PressedEventHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self,args):
        ...
        WormGear_Type.listItems.add('斜齿轮',True,'Resources/Helical')
        WormGear_Type.listItems.add('滚切直齿',False,'Resources/HobbedWorm')
        WormGear_Type.tooltipDescription = "斜齿轮：蜗轮为斜齿轮。\n滚切直齿：蜗杆决定蜗轮齿形，保持直齿轮几何/尺寸。"
        ...
        inputs.addBoolValueInput('LeftThreaded', '左旋', True, '', get(self, 'LeftThreaded', False))
        inputs.addValueInput('Module', '模数 [mm]', 'mm', adsk.core.ValueInput.createByReal(get(self, 'Module', .03)))
        pitch = inputs.addValueInput('Pitch', '径节 [in]', 'in', adsk.core.ValueInput.createByReal(get(self, 'Pitch', 23.460456)))
        pitch.isVisible = False

        inputs.addIntegerSpinnerCommandInput('Z', '齿数 [ ]', 6, 250, 1, get(self, 'Z', 17))
        inputs.addValueInput('WormLength_mm','蜗杆长度 [mm]','mm', adsk.core.ValueInput.createByReal(get(self, 'WormLength_mm', 1)))
        WormLength_in = inputs.addValueInput('WormLength_in','蜗杆长度 [in]','in', adsk.core.ValueInput.createByReal(get(self, 'WormLength_in', 1)))
        WormLength_in.isVisible = False

        inputs.addValueInput('WormGearHeight_mm', '蜗轮高度 [mm]', 'mm', adsk.core.ValueInput.createByReal(get(self, 'WormGearHeight_mm', 1)))
        WormGearHeight_in = inputs.addValueInput('WormGearHeight_in', '蜗轮高度 [in]', 'in', adsk.core.ValueInput.createByReal(get(self, 'WormGearHeight_in', 1)))
        WormGearHeight_in.isVisible = False

        inputs.addFloatSpinnerCommandInput('PressureAngle', '压力角 [°]', 'deg', 14.5, 30, 0.5, get(self, 'PressureAngle', 14.5))
        inputs.addValueInput('WormDriveRadius_mm','蜗杆驱动半径 [mm]','mm', adsk.core.ValueInput.createByReal(get(self, 'WormDriveRadius_mm', 0.5)))
        WormDriveRadius_in = inputs.addValueInput('WormDriveRadius_in','蜗杆驱动半径 [in]','in', adsk.core.ValueInput.createByReal(get(self, 'WormDriveRadius_in', 0.5)))
    ...
    def notify(self, args):
        ...
        try:
            ...
            if changedInput.id == 'standard':
                ...
                if changedInput.selectedItem.name == '滚切直齿':
                    inputs2.itemById('HelicalSystem').isVisible = False
                elif changedInput.selectedItem.name == '斜齿轮':
                    inputs2.itemById('HelicalSystem').isVisible = True
...
def run(context):
    ...
    try:
        ...
        cmdDef3=ui.commandDefinitions.addButtonDefinition('NC3','非标内直齿轮','创建非标准内直齿轮。','Resources/NoStdr')
        cmdDef4=ui.commandDefinitions.addButtonDefinition('NC4','单/双斜齿轮','创建标准单/双斜齿轮。','Resources/Helical')
        cmdDef5=ui.commandDefinitions.addButtonDefinition('NC5','单/双非标内斜齿轮','创建非标准单/双内斜齿轮。','Resources/NoStdr')
        cmdDef6=ui.commandDefinitions.addButtonDefinition('NC6','单/双内斜齿轮','创建标准单/双内斜齿轮。','Resources/InteriorGear')
        cmdDef7=ui.commandDefinitions.addButtonDefinition('NC7','直/斜齿条','创建齿条。','Resources/Rack')
        cmdDef8=ui.commandDefinitions.addButtonDefinition('NC8','90°锥齿轮','创建一对标准90°相交轴锥齿轮。','Resources/Conicos')
        cmdDef9=ui.commandDefinitions.addButtonDefinition('NC9','变位直齿轮','使用“变位”创建直齿轮。','Resources/Recto')
        cmdDef10=ui.commandDefinitions.addButtonDefinition('NC10','变位斜齿轮','使用“变位”创建斜齿轮。','Resources/Helical')
        cmdDef11=ui.commandDefinitions.addButtonDefinition('NC11','蜗轮蜗杆传动','创建蜗轮蜗杆传动装置。','Resources/WormGear')
